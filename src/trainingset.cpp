...
pair<Mat, Mat> TrainingSet::compute(double mmPerPixel) {
    ...
    for (SetEntry &m : image_groups) {
        ...
        for (Mat &img: m.images) {
            ...
            bool circle = fe.calculate_if_circle(gray_image, m.threshold_value);
            bool square = fe.calculate_if_square(gray_image, m.threshold_value);
            int surface_area = fe.calculate_surface_area(gray_image, m.threshold_value);
            int length = fe.calculate_length(gray_image, m.threshold_value);
            int width = fe.calculate_width(gray_image, m.threshold_value);
            double bending_energy = fe.calculate_bending_energy(gray_image, m.threshold_value);
            double keypoint_count = fe.calculate_keypoints(gray_image, m.threshold_value);
            double aspect_ratio = fe.calculate_aspect_ratio(gray_image, m.threshold_value);
            double avg_gray = fe.calculate_average_gray_value(gray_image, m.threshold_value);
...
FeatureExtractor::FeatureExtractor(double mmPerPixel)
{
	this->mmPerPixel = mmPerPixel;
}

// give this a gray_image
int FeatureExtractor::calculate_surface_area(Mat &img, int thresh) {
    Mat bin;
    ...
    return sum * mmPerPixel;
}

// give this a gray_image
bool FeatureExtractor::calculate_if_square(Mat &img, int thresh) {
...
bool FeatureExtractor::calculate_if_circle(Mat &img, int thresh) {
    Mat gray_image(img);
...
int FeatureExtractor::calculate_length(Mat & img, int thresh)
{
	Mat bin;
	threshold(img, bin, thresh, 255, THRESH_BINARY);
	...
	return boundaryRotatedBox.size.height * mmPerPixel;
}

int FeatureExtractor::calculate_width(Mat & img, int thresh)
{
	Mat bin;
	threshold(img, bin, thresh, 255, THRESH_BINARY);
	...
	return boundaryRotatedBox.size.width * mmPerPixel;
}

double FeatureExtractor::calculate_keypoints(Mat &img, int thresh) {
...
double FeatureExtractor::calculate_aspect_ratio(Mat &img, int thresh) {
    Mat bin;
    threshold(img, bin, thresh, 255, THRESH_BINARY);
    vector<vector<Point> > contours;
    vector<Vec4i> hierarchy;
...
double FeatureExtractor::calculate_average_gray_value(Mat &img, int thresh) {
...
double FeatureExtractor::calculate_bending_energy(Mat & img, int thresh)
{
	Mat bin;
	threshold(img, bin, thresh, 255, THRESH_BINARY);
...
int FeatureExtractor::calculate_perimeter(Mat & img, int thresh)
{
	Mat bin;
	threshold(img, bin, thresh, 255, THRESH_BINARY);
